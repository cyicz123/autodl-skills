...
export async function main(argv = process.argv.slice(2), io: IoLike = process): Promise<number> {
  try {
    ...
    switch (namespace) {
      case "elastic":
        return await elasticMain(rest, io as typeof process);
      case "pro":
        return await proMain(rest, io as typeof process);
      default:
        throw new ApiError(
          "config_error",
          `Unknown namespace: ${String(namespace)}. Use "elastic" or "pro".`,
          {},
          1,
        );
    }
