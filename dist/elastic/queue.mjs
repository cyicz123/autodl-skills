...
import { createDeployment, fetchAllImages, fetchGpuStock } from "./api.mjs";
import { validateElasticConfig } from "./schema.mjs";
const MAX_SUBMIT_RETRIES = 3;
export async function queueSubmit(config, options) {
    ...
    const gpuNames = template.gpu_name_set;
    ...
    while (true) {
        if (hasIdleGpu(stock, gpuNames, gpuNum)) {
            try {
                const deployment = await createDeployment(options, config);
...
function hasIdleGpu(stock, gpuNames, gpuNum) {
    return gpuNames.some((name) => (stock[name]?.idle ?? 0) >= gpuNum);
}
function submissionError(apiResponse) {
